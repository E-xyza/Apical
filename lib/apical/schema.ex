defmodule Apical.Schema do
  ...
  defp verify_operation_id_verb(_path, {_verb, %{"operationId" => name}}, so_far) do
    Tools.assert(name not in so_far, "that operationIds are unique: (got more than one `#{name}`)")
  ...
  defp verify_operation_id_verb(path, {verb, _}, _) do
    ...
    Tools.assert(false, "that all operations have an operationId: (missing for operation at `#{path}`)")
  end
end
